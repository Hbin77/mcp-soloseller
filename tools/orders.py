...
async def get_orders(channel: str = "all", days: int = 7) -> dict[str, Any]:
    ...
    if channel in ["all", "naver"]:
        if creds and creds.naver_configured:
            ...
            errors.append("네이버 API 키가 설정되지 않았습니다. https://soloseller.cloud 에서 회원가입 후 설정 페이지에서 API 키를 등록해주세요.")
    ...
    if channel in ["all", "coupang"]:
        if creds and creds.coupang_configured:
            ...
            errors.append("쿠팡 API 키가 설정되지 않았습니다. https://soloseller.cloud 에서 회원가입 후 설정 페이지에서 API 키를 등록해주세요.")
