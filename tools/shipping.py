...
async def issue_invoice(
    ...
    if not creds:
        return {"success": False, "error": "인증 정보가 없습니다. https://soloseller.cloud 에서 회원가입 후 토큰을 발급받아 사용해주세요."}
    ...
    if not creds.sender_configured:
        return {"success": False, "error": "발송인 정보가 설정되지 않았습니다. https://soloseller.cloud 의 설정 페이지에서 발송인 정보를 등록해주세요."}
...
async def register_invoice(
    ...
    if channel == "naver":
        if not creds.naver_configured:
            return {"success": False, "error": "네이버 API 키가 설정되지 않았습니다. https://soloseller.cloud 의 설정 페이지에서 API 키를 등록해주세요."}
        ...
        if not creds.coupang_configured:
            return {"success": False, "error": "쿠팡 API 키가 설정되지 않았습니다. https://soloseller.cloud 의 설정 페이지에서 API 키를 등록해주세요."}
